...
const referenceArray = referenceData as Reference[];
        ...
        <section>
            ...
            {referenceArray.map((r: Reference, index: number) => (
                <div key={r.name} className={styles.reference}>
                    <h2>{r.name}</h2>
                    <h3>{r.title}</h3>
                    {r.review && <blockquote cite={r.linkedIn}>"{r.review}"</blockquote>}
                    <p><a href={`mailto:${r.email}`}>{r.email}</a></p>
                    {r.linkedIn && <a target="_blank" rel="noopener noreferrer" href={r.linkedIn}>
